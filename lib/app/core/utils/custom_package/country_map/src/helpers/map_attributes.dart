...
class MapAttributes {
  ...
  Size latLngToPixelnewMethod(LatLong position) {
    // Convert longitude to x coordinate
    double x = ((position.longitude + 180) / 360) * mapWidth;

    // Convert latitude to y coordinate (using Mercator projection)
    double latRad = position.latitude * pi / 180;
    double y = (1 - (log(tan(latRad) + 1 / cos(latRad)) / pi)) * mapHeight / 2;
    //u8  double y = ((90 - position.latitude) / 180) * mapHeight;
  ...
  Size latLongToPixels(LatLong position) {
    ...
    final double relativeX = (position.longitude - topLeft.longitude);
